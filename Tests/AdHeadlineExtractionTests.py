...
class HeadlineTester(TestCase):
    def test_count_params(self):
        headline_to_test = pd.DataFrame({'ad_id': ['1'], 'headline_text':
            ["Over 55s in ${city:capitalized}$ Can Now Use This Free Equity Release Calculator"]})
    ...
    def test_count_zero_params(self):
        headline_to_test = pd.DataFrame({'ad_id': ['1'], 'headline_text':
            ["Over 55s in London Can Now Use This Free Equity Release Calculator"]})
        ad_extractor = AdExtractor(headline_to_test)
    ...
    def test_contains_ad_id(self):
        headline_to_test = pd.DataFrame({'ad': ['1'], 'headline_text':
            ["Over 55s in London Can Now Use This Free Equity Release Calculator"]})
        with self.assertRaises(KeyError):
            AdExtractor(headline_to_test)

    def test_contains_headline_text(self):
        headline_to_test = pd.DataFrame({'ad_id': ['1'], 'headline':
            ["Over 55s in London Can Now Use This Free Equity Release Calculator"]})
        with self.assertRaises(KeyError):
            AdExtractor(headline_to_test)
